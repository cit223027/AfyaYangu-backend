...
import {SpeakContext, SpeakDivContext, SpeakPageContext} from "@/components/speech/SpeakContext.ts";
    ...
    return (
        <div id={id} className={`${className} group flex items-center`}>
            ...
            <div className="ml-2 transform -translate-x-full opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-transform duration-300 ease-in-out">
                <Button className="rounded-full p-2" variant="ghost" onClick={() => startSpeaking(speakDivContent.map((content) => content.text))}>
