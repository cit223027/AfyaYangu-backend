...
class MedicalCenterFirebaseRepository:
    ...
    @staticmethod
    def get_medical_center_collection_reference(firebase_app: firebase_admin.App):
        return firestore.client(firebase_app).collection('medical_center')

    @staticmethod
    def upsert_medical_center(firebase_app, medical_center: MedicalCenter) -> str:
        """
        ...
        if medical_center.medical_center_id is None:
            ...
            _, new_ref = MedicalCenterFirebaseRepository.get_medical_center_collection_reference(firebase_app).add(medical_center.to_dict())
            new_ref.update(
            ...
            MedicalCenterFirebaseRepository.get_medical_center_collection_reference(firebase_app).document(medical_center.medical_center_id).update(medical_center.to_dict())
            return medical_center.medical_center_id

    @staticmethod
    def get_all_medical_centers(firebase_app) -> List[MedicalCenter]:
        """
    ...
    @staticmethod
    def get_medical_center(firebase_app, medical_center_id: str) -> Optional[MedicalCenter]:
        """
        ...
        """
        doc_ref = MedicalCenterFirebaseRepository.get_medical_center_collection_reference(firebase_app).document(medical_center_id)
        doc = doc_ref.get()
        if doc.exists:
    ...
    @staticmethod
    def delete_medical_center(firebase_app, medical_center_id: str):
        """
    ...
    @staticmethod
    def search_medical_center_by_name(firebase_app, name: str) -> List[MedicalCenter]:
        """
        Searches for medical centers by name (partial match)
        :param name: Name to search for
        :return: List of MedicalCenter objects that match the name
        """
        query = MedicalCenterFirebaseRepository.get_medical_center_collection_reference(firebase_app).where('name', '>=', name).where('name', '<=', name + '\uf8ff')
        docs = query.stream()
    ...
    def filter_medical_centers_by_location(
        ...
        """
        all_centers = MedicalCenterFirebaseRepository.get_medical_center_collection_reference(firebase_app).stream()
        nearby_centers = []
